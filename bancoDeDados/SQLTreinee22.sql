...
SELECT c.name, COALESCE(SUM(p.amount),0)  FROM products  p
right join categories c on c.id = p.id_categories
group by c.id 

/*Apresentar um relatório com as 4 categorias que mais tiveram produtos
comprados.*/
SELECT c.id, c.name, SUM(p.amount) sum  FROM products  p
left join categories c on c.id = p.id_categories
group by c.id
order by sum(p.amount) DESC LIMIT 4
